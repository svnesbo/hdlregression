...
# ID_VHDL_COMPONENT = r'(^|\W)component\s+[is]?'
...
ID_VHDL_END_ARCH = r'\bend\s*(;|architecture\s*(;|\w+\s*;))'  # r'[\s\r\n]?end[\s+]architecture[\s*]?'
RE_VHDL_END_ARCH = re.compile(ID_VHDL_END_ARCH, flags=re.IGNORECASE)

ID_VHDL_END_PKG = r'\bend\s*(;|package\s*(;|\w+\s*;))'  # r'[\s\r\n]?end[\s+]package[\s*]?'
RE_VHDL_END_PKG = re.compile(ID_VHDL_END_PKG, flags=re.IGNORECASE)

ID_VHDL_END_PKG_BODY = r'\bend\s*(;|package[\s+]body\s*(;|\w+\s*;))'  # r'[\s\r\n]?end[\s+]package[\s+]body[\s*]?'
RE_VHDL_END_PKG_BODY = re.compile(ID_VHDL_END_PKG_BODY, flags=re.IGNORECASE)

ID_VHDL_END_CONTEXT = r'\bend\s*(;|context\s*(;|\w+\s*;))'  # r'[\s\r\n]?end[\s+]context[\s*]?'
...
ID_VHDL_RESERVED = ["abs", "configuration", "impure", "null", "rem", "type", "access", "constant", "in", "of",
                    "report", "unaffected", "after", "disconnect", "inertial", "on", "return", "units", "alias",
                    "downto", "inout", "open", "rol", "until", "all", "else", "is", "or", "ror", "use", "and",
                    "elsif", "label", "others", "select", "variable", "architecture", "end", "library", "out",
                    "severity", "wait", "array", "entity", "linkage", "package", "signal", "when", "assert",
                    "exit", "literal", "port", "shared", "while", "attribute", "file", "loop", "postponed",
                    "sla", "with", "begin", "for", "map", "procedure", "sll", "xnor", "block", "function", "mod",
                    "process", "sra", "xor", "body", "generate", "nand", "pure", "srl", "buffer", "generic",
                    "new", "range", "subtype", "bus", "group", "next", "record", "then", "case", "guarded", "nor",
                    "register", "to", "component", "if", "not", "reject", "transport"]
...
ID_VERILOG_RESERVED = ["always", "end", "ifnone", "or", "rpmos", "tranif1", "and", "endcase", "initial",
                       "output", "rtran", "tri", "assign", "endmodule", "inout", "parameter", "rtranif0",
                       "tri0", "begin", "endfunction", "input", "pmos", "rtranif1", "tri1", "buf", "endprimitive",
                       "integer", "posedge", "scalared", "triand", "bufif0", "endspecify", 'join', 'primitive',
                       'small', 'trior', 'bufif1', 'endtable', 'large', 'pull0', 'specify', 'trireg',
                       'case', 'endtask', 'macromodule', 'pull1', 'specparam', 'vectored', 'casex', 'event',
                       'medium', 'pullup', 'strong0', 'wait', 'casez', 'for', 'module', 'pulldown', 'strong1',
                       'wand', 'cmos', 'force', 'nand', 'rcmos', 'supply0', 'weak0', 'deassign', 'forever',
                       'negedge', 'real', 'supply1', 'weak1', 'default', 'for', 'nmos', 'realtime', 'table',
                       'while', 'defparam', 'function', 'nor', 'reg', 'task', 'wire', 'disable', 'highz0', 'not',
                       'release', 'time', 'wor', 'edge', 'highz1', 'notif0', 'repeat', 'tran', 'xnor', 'else',
                       'if', 'notif1', 'rnmos', 'tranif0', 'xor']
